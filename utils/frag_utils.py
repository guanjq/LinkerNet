...
def remove_dummys(smi_string):
    return Chem.MolToSmiles(Chem.RemoveHs(
        AllChem.ReplaceSubstructs(Chem.MolFromSmiles(smi_string), Chem.MolFromSmiles('*'), Chem.MolFromSmiles('[H]'),
                                  True)[0]))
            ...
            for i, mol in enumerate(mols):
                ...
                if i == 1:
                    ...
                    if len(dummy_atom_idxs) != 2:
                        ...
                        path_length = len(
                            Chem.rdmolops.GetShortestPath(mol, dummy_atom_idxs[0], dummy_atom_idxs[1])) - 2
...
def get_frags(full_mol, clean_frag):
    full_mol = copy.deepcopy(full_mol)
    matches = list(full_mol.GetSubstructMatches(clean_frag))
    linker_len = full_mol.GetNumHeavyAtoms() - clean_frag.GetNumHeavyAtoms()
...
def compute_distance_and_angle(mol, smi_linker, smi_frags):
    try:
        ...
        for frag_match, linker_match in product(frags_matches, linker_matches):
            ...
            l_match = [idx for num, idx in enumerate(linker_match) if
                       linker.GetAtomWithIdx(num).GetAtomicNum() != 0 and idx not in f_match]
...
def plot_hist(results, prop_name, labels=None, alpha_val=0.5):
    ...
    if prop_name in ['heavy_atoms', 'num_C', 'num_N', 'num_O', 'num_F', 'hba', 'hbd', 'ring_ct', 'avg_ring_size',
                     'rot_bnds', 'stereo_cnts', "Linker lengths"]:
    ...
    for processed, res in enumerate(results):
        ...
        for m in res:
            ...
            clean_frag = Chem.RemoveHs(
                AllChem.ReplaceSubstructs(Chem.MolFromSmiles(m[0]), du, Chem.MolFromSmiles('[H]'), True)[0])
...
def check_2d_filters_dataset(fragmentations, n_cores=1, pains_smarts_loc='./wehi_pains.csv'):
    ...
    with Parallel(n_jobs=n_cores, backend='multiprocessing') as parallel:
        results = parallel(
            delayed(check_2d_filters)(toks, pains_smarts, count, True) for count, toks in enumerate(fragmentations))
...
def calc_filters_2d_dataset(results, pains_smarts_loc, n_cores=1):
    ...
    with Parallel(n_jobs=n_cores, backend='multiprocessing') as parallel:
        filters_2d = parallel(delayed(calc_2d_filters)([toks[2], toks[-1], toks[1]], pains_smarts) for toks in results)
    ...
    for res in results:
        ...
        for m in res:
            ...
            if DataStructs.TanimotoSimilarity(AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(m[0]), 2, 2048),
                                              AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(m[2]), 2,
                                                                                    2048)) == 1:
                recovered += 1
                break
...
def recovered_by_smi_canon(results):
    ...
    for res in results:
        ...
        for m in res:
            ...
            if Chem.MolToSmiles(Chem.RemoveHs(Chem.MolFromSmiles(m[0]))) == Chem.MolToSmiles(
                    Chem.RemoveHs(Chem.MolFromSmiles(m[2]))):
                recovered += 1
                break
    return recovered / total
...
def join_frag_linker(linker, st_pt, random_join=True):
    if linker == "":
        ...
        return Chem.MolToSmiles(Chem.RemoveHs(
            AllChem.ReplaceSubstructs(Chem.MolFromSmiles(st_pt), du, Chem.MolFromSmiles('[H]'), True)[0])).split('.')[0]
...
def SC_RDKit_frag_mol(gen_mol, ref_mol, start_pt):
    try:
        ...
        du = Chem.MolFromSmiles('*')
        clean_frag = Chem.RemoveHs(
            AllChem.ReplaceSubstructs(Chem.MolFromSmiles(start_pt), du, Chem.MolFromSmiles('[H]'), True)[0])
